...
func DialSSH(channel ssh.Channel, user string, server string, port int) {
	...
	if err != nil {
		log.Panic("Unable to setup stdin for session: %v", err)
	}
	...
	if err != nil {
		log.Panic("Unable to setup stdout for session: %v", err)
	}
	...
	if err != nil {
		if err, ok := err.(*ssh.ExitError); ok {
			fmt.Errorf("command exited with: %v", err)
		} else {
	...
	if err != nil {
		log.Print("Error closing the ssh channel")
	}
